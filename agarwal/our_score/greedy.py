...
def runGreedy(trajs, distPairs, strajCov, ptStraj, strajPth, trajCov, c1, c2, c3):
    ...
    # Initialize a priority queue of trajs, with coverage to cost ratio of a singleton set, i.e., |T| / (|T|/n*c2).
    queue2 = heapdict()
    for trID, cov in trajCov.iteritems():
        queue2[trID] = -(1.0*points_count)/(1.0*c2)
    ...
    while numUnprocessedPts[0] > 0:
        ...
        for path in affectedPathlets:
            # BUG: this recomputation fundamentally doesn't work. As gamma might change, we can't just subtract the contribution of covered curves.
            # computeOptStrajsAdvanced(path, distPairs, pthOptStrajs, strajCov, c1, c3, len(ptStraj), affectedTrajs)
            computeOptStrajsAdvanced(path, distPairs, pthOptStrajs, strajCov, c1, c3, len(ptStraj), pthOptStrajs[path].keys())
