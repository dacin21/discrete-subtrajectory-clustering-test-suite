...
private:
	...
	std::size_t sweep_line(std::size_t x_start, std::size_t x_end, std::size_t target_num_curves) {
		...
		while (*top_y != x_end) {
			...
			if (flag_a && !flag_b) {
				...
				if (num_curves >= target_num_curves) {
					std::cerr << "[" << x_start << "," << x_end << "] -> " << num_curves << "\n";
					return num_curves;
				}
			} else {
				top_y = top_y - num_cols;
	...
	*/
	...
	SubTrajectoryClustering(InputIterator polyline_first,
		...
		while (ls < (num_cols - 1)) {
			// Weird heuristic?
			// std::size_t lt = ls + 1;
			std::size_t num_clusters = sweep_line(ls, lt, num_cluster_threshold);
			...
			if ((num_clusters >= num_cluster_threshold) && (lt < num_cols)) {
				while ((num_clusters >= num_cluster_threshold) && (++lt < num_cols)) {
					...
					num_clusters = sweep_line(ls, lt, num_cluster_threshold);
				}
				if ((lt - ls) > max_length) {
					max_length = lt - ls - 1;
					start_idx = ls;
			...
			// This seems too hasty. Weird heuristic?
			// ls = lt;
			// Proper two-pointer sweep.
			++ls;
		}
	}
