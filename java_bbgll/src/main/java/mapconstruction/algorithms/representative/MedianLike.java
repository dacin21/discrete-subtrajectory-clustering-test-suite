...
public final class MedianLike {
    ...
    // vertex closest to start
    static Pair<Point2D, Pair<Subtrajectory, Integer>> closestVertex(Collection<Subtrajectory> trajectories, Point2D start) {
        Pair<Point2D, Pair<Subtrajectory, Integer>> closest = null;
        for (Subtrajectory t : trajectories) {
            for (int i = 0; i < t.numPoints(); i++) {
                if (closest == null)
                    closest = new Pair(t.getPoint(i), new Pair(t, i));
                if (start.distanceSq(t.getPoint(i)) < start.distanceSq(closest.getFirst()))
                    closest = new Pair(t.getPoint(i), new Pair(t, i));
    ...
    // clip the repr to the closest
    static List<Point2D> clipToClosest(List<Point2D> repr, Point2D end) {
        Set<Subtrajectory> ts = new HashSet(Arrays.asList(new Subtrajectory(new FullTrajectory(repr))));
        int i = closestVertex(ts, end).getSecond().getSecond();
        List<Point2D> res = repr.subList(0, Math.max(i, 1));
        res.add(end);
...
//	Pair<Set<Subtrajectory>,List<Point2D>> initialize(Set<Subtrajectory> trajectories, Point2D start) { 
//		Pair<Point2D,Pair<Subtrajectory,Integer>> p = closestVertex(trajectories,start);
//		
//		List<Point2D> repr = Arrays.asList(start,p.getFirst());
//		
//		Subtrajectory t = p.getSecond().getFirst();
//		int i = p.getSecond().getSecond();
//		t.dropInPlace(Math.max(0, i));
//		
//		return new Pair(trajectories)
//	}
//	
//	
    ...
    public static Subtrajectory representativeTrajectory(Set<Subtrajectory> trajectories, Point2D start, Point2D end) {
        ...
        Subtrajectory startT = null;
        for (Subtrajectory t : trajectories) {
            Subtrajectory tp = appendEndPoints(t, start, end);
            ts.add(tp);
            if (t.getFirstPoint().equals(start)) {
                startT = tp;
            }
        }
        ...
        if (startT == null) {
            Pair<Point2D, Pair<Subtrajectory, Integer>> x = closestVertex(ts, start);
            repr.add(x.getFirst());
            startT = x.getSecond().getFirst();
            x.getSecond().getFirst().dropInPlace(x.getSecond().getSecond());
        }
...
//		P<Subtrajectory> s = new P(start,t);
        ...
        return new Subtrajectory(new FullTrajectory(clipToClosest(repr.subList(0, repr.size() - 1), end)));
